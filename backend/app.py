...
_allowed = "*" if _cors_env == "*" else {o.strip() for o in _cors_env.split(",") if o.strip()}
CORS(app, resources={r"/*": {"origins": _allowed if _allowed != "*" else "*"}})

@app.after_request
def add_cors_headers(resp):
    origin = request.headers.get("Origin")
    if _allowed == "*" or (origin and origin in _allowed):
        resp.headers["Access-Control-Allow-Origin"] = "*" if _allowed == "*" else origin
...
DB_URL = os.getenv("DATABASE_URL") or os.getenv("POSTGRES_URL") or "sqlite:///reviews.db"
...
# Access codes (override with env)
...
API_PREFIX = os.getenv("API_PREFIX", "/api")  # for display
...
def root():
    return jsonify({
        ...
        "endpoints": {
            f"GET {API_PREFIX}/reviews": "List reviews",
            f"POST {API_PREFIX}/auth/login": "Body: {code, client_id?} -> JWT",
            f"POST {API_PREFIX}/reviews": "Create review (JWT)",
            f"DELETE {API_PREFIX}/reviews/<id>": "Delete (owner/admin)"
        }
    })
